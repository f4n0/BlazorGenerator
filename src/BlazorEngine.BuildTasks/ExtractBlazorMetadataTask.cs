...
public class ExtractBlazorMetadataTask : Task
{
  ...
  [Output] public int MenuItemCount { get; set; }

  [Output] public int FooterLinkCount { get; set; }

  [Output] public int PageActionCount { get; set; }

  [Output] public int GridActionCount { get; set; }

  [Output] public int ContextMenuCount { get; set; }
  ...
  public override bool Execute()
  {
    ...
    try
    {
      ...
      Log.LogMessage(MessageImportance.Normal,
        ...
        Path.GetFileName(AssemblyPath), MenuItemCount, FooterLinkCount, PageActionCount, GridActionCount,
        ContextMenuCount);
  ...
  private void ProcessType(TypeDefinition type, MetadataFormat.AssemblyMetadataFile metadata)
  {
    ...
    if (type.HasCustomAttributes)
      foreach (var attr in type.CustomAttributes)
      {
        ...
        else if (attrName == FooterLinkAttr) metadata.FooterLinks.Add(ExtractFooterLinkEntry(type, attr));
      }
    ...
    if (type.HasMethods)
    {
      ...
      foreach (var method in type.Methods)
      {
        ...
        foreach (var attr in method.CustomAttributes)
        {
          ...
          else if (attrName == ContextMenuAttr) contextMenus.Add(ExtractContextMenuEntry(method, attr));
        }
      }
