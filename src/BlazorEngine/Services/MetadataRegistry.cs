...
namespace BlazorEngine.Services
{
  ...
  public static class MetadataRegistry
  {
    ...
    private static readonly ConcurrentDictionary<Type, (MethodInfo Method, PageActionAttribute Attribute)[]> _pageActions = new();
    private static readonly ConcurrentDictionary<Type, (MethodInfo Method, GridActionAttribute Attribute)[]> _gridActions = new();
    private static readonly ConcurrentDictionary<Type, (MethodInfo Method, ContextMenuAttribute Attribute)[]> _contextMenus = new();
    ...
    /// </summary>
    public static int PreComputedAssemblyCount => _registrations.Count(r => r.Value.Source == RegistrationSource.BuildTime);
    ...
    private static void ScanUnregisteredAssemblies()
    {
      ...
      foreach (var assembly in AppDomain.CurrentDomain.GetAssemblies())
      {
        ...
        bool referencesBlazorEngine = false;
        try
        {
    ...
    private static void ResolveViaReflection(Assembly assembly)
    {
      try
      {
        ...
        {
          // Menu items
          var menuAttr = type.GetCustomAttribute<AddToMenuAttribute>(inherit: true);
          if (menuAttr != null)
            _menuItems.Add((type, menuAttr));

          // Footer links
          var footerAttr = type.GetCustomAttribute<FooterLinkAttribute>(inherit: true);
          if (footerAttr != null)
      ...
      where TAttribute : Attribute
    {
      ...
      foreach (var m in methods)
      {
        ...
        var attr = m.GetCustomAttribute<TAttribute>(inherit: true);
        if (attr != null)
          list.Add((m, attr));
      }
