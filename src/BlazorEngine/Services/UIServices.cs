...
public partial class UIServices(
  BlazorEngineLogger logger,
  IDialogService dialogService,
  ProgressService progressService,
  IKeyCodeService keyCodeService,
  LockUIService lockService)
{
  ...
  public async Task<T?> OpenModal<T>(Type pageType, T data) where T : class
  {
    ...
    if (result.Data is not null && !result.Cancelled) return result.Data as T;
    return null;
  }
    ...
    long maxFileSize = 10 * 1024 * 1024)
  {
    ...
    if (result.Data is not null && !result.Cancelled)
    {
      ...
      if (err is not null) throw new Exception(err);
      return ret;
    }
  ...
  public async Task<UserInputData?> UserInput(UserInputData userInputData)
  {
    ...
    var result = await dialogResult.Result.ConfigureAwait(true);
    if (result.Data is not null && !result.Cancelled) return result.Data as UserInputData;
    return null;
  }
  ...
  public async Task<T?> OpenPanel<T>(Type pageType, T data) where T : class
  {
    ...
    if (result.Data is not null) return result.Data as T;
    return null;
  }
