...
namespace BlazorEngine.Utils
{
  static class AttributesUtils
  {
    ...
    {
      internal static readonly ConcurrentDictionary<Type, (PropertyInfo Property, TAttribute Attribute)[]> Properties = new();
      ...
      internal static readonly Lazy<(Type Type, TAttribute Attribute)[]> Models =
        new(BuildModels, System.Threading.LazyThreadSafetyMode.ExecutionAndPublication);
      ...
      where TAttribute : Attribute
    {
      ...
      return Cache<TAttribute>.Properties.GetOrAdd(type, static t =>
      {
        ...
        foreach (var p in props)
        {
          ...
          var attr = p.GetCustomAttribute<TAttribute>(inherit: true);
          if (attr != null)
            list.Add((p, attr));
        }
    ...
    internal static IEnumerable<(Type Type, TAttribute Attribute)> GetModelsWithAttribute<TAttribute>()
      where TAttribute : Attribute
      => Cache<TAttribute>.Models.Value;

    internal static IEnumerable<TAttribute> GetModelsWithAttribute<TAttribute>(object obj)
      where TAttribute : Attribute
      => obj.GetType().GetCustomAttributes<TAttribute>(inherit: true);
  }
}